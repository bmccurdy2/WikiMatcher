...
public class CombinedAlign extends URIAlignment{
	
	private Levenstein levensteinCompare = new Levenstein();
	private OWLData owldata;
	private Utilities util;
	
	CombinedAlign(String ontology){
		this.owldata = new OWLData(ontology);
		this.util = new Utilities(ontology, "combined");
	}
	
	public void CombinedAlignment(URI uri1, URI uri2, String name1, String name2, PrintWriter output) throws Exception{
...
//		if(x.get(0).equals("PAGE-STATUS::found") || x.get(0).equals("PAGE-STATUS::redirected") || y.get(0).equals("PAGE-STATUS::found") || y.get(0).equals("PAGE-STATUS::redirected")){
		if (x.get(0).equals("PAGE-STATUS::redirected") && y.get(0).equals("PAGE-STATUS::redirected")){
			return 0.0;
		} else {
			tempX = x.get(0);
			x.remove(0);
...
//			data.remove(0);
			newdata.add("PAGE-STATUS::search");
			for (String s : data){
				if (s.startsWith("T:")){
					s = s.replaceAll("T:", "");
					s = s.replaceAll("\\(.*?\\) ?", "");
	...
	public static void doConference() throws Exception{
		...
		CombinedAlign aligner = new CombinedAlign("conference_v1");
		...
		for (int i = 0; i < files.length; i++) {
			...
			for (int j = i; j < files.length; j++) {
				...
				aligner.CombinedAlignment(f1.toURI(), f2.toURI(), name1, name2, output);
	...
	public static void doAnatomy() throws Exception{
		...
		CombinedAlign aligner = new CombinedAlign("anatomy");
	...
	public static void doGeo() throws Exception{
		...
		CombinedAlign aligner = new CombinedAlign("geo");
